...
- **보류 (단일 경로)**: 지금 쓸 수 없으면 — 범위 미확인·금지영역·경로 부재·DEBT 해소가 다음 작업으로 넘어감·보안 미배포 — log `아카이브 보류(<사유>)` 1행 + NEXT.md `보류·이월`에 재개 조건 등재 + 사용자 보고. DEBT 이월은 **이월을 결정한 시점에 즉시** 이 경로로 기록한다(검증 완료 전이어도 — 기록만, 카드 쓰기는 해소 후). 다음 사이클 마감(또는 조건 해소) 때 **보류 항목부터** 처리한다. 조용히 건너뛰지 않는다.
- **보안 결함은 출처 무관 패치·배포 완료 후에만** 아카이브한다(공개 repo — 미패치 취약점 공개 금지, 하네스 훅 우회 포함) — 그 전엔 위 보류 경로.
