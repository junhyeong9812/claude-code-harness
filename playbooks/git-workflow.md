...
> 트리거: **원격(`git remote`)이 있는 L1(구현) 작업** (core §6.5). 경량 경로(오타·자명)·로컬 전용 repo는 제외. 개발 단계(§3.3) 진입 시 §2부터, 기록 단계(§3.5) 종료 후 §4.
...
> **기본 흐름 = 브랜치 항상 생성, 이슈는 사용자가 요청할 때만**(2026-07-03 사용자 결정). 처리 전 구조/방식을 물을 때 그에 맞게 확인한다.

1. **작업 브랜치 생성 (로컬, 항상)** — main 직접 작업 금지. **base·브랜치명을 사용자에게 확인**(기본 base=기본 브랜치, 이름=이슈 있으면 `N-<slug>`·없으면 `<slug>`) 후 `git switch -c <branch> <base>`. **로컬 분기만** — 원격 브랜치 생성은 push와 동급 외부 발행이라 쓰지 않는다(승인 경계 §6.5).
2. **이슈 발행 (요청 시만)** — 사용자가 이슈를 원하면 제목=작업명, 본문=task.md §1 정의 6칸 요약(목표·경계·검증)으로 **확인 후 생성**(`gh issue create` / `glab issue create`), 번호 `N` 확보. 요청이 없으면 이슈 없이 브랜치·로컬 커밋으로 진행하고 완료 시 push/PR을 §4대로 확인받는다.

## 3. 개발 (페이즈 커밋)

- §3.2 페이즈 분할대로: 페이즈마다 빌드/테스트 통과 후 커밋. **code/docs 분리**(§6.4 scope-guard), **AI trailer 금지**, **검증 출처 금지**(§6.4 — 근거는 task-process·review-log).
