...
| 상태 파일 | MODE·PENDING_GATE·WRITE_PHASE 갱신은 실패가 은폐되지 않는다 | pre: 상태 파일 존재 / post: 갱신 확인 또는 fail-closed | 갱신 실패 → **fail-closed**(exit 2 — PreToolUse 차단·PostToolUse 강피드백. 설계검증 #23 채택, 리뷰 P1-03에서 "경고만"이던 이 행을 design v2와 정합) |
