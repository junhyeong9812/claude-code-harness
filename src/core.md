...
| 리뷰 | 셀프체크 | **듀얼 1패스**(Opus 워커 ∥ codex → 종합 → 감사 → post-fix 재점검 1회) | **듀얼 리뷰 루프**(≤3) + 설계 선검증(spec ②에 코드베이스에 없던 불변식이 있을 때만 — 생략 시 log 1행: ② 항목별 기존 강제 위치 file:line) + blind 테스트 워커 |
| 테스트 설계 | 구현자 | 분리 패스(spec 먼저) | 별도 워커 — 구현 diff 미열람 계약 |

- ★ **산출물 유형 분기 (2026-08-06 실측 — jun-bank)**: 산출물이 **설계 문서** — *그 내용이 이후 코드로 구현되어 컴파일·테스트가 검증하게 될 문서*(설계·명세·계약) — 이면 리뷰 상한 = **설계 선검증(신설 불변식 덩어리 의무 — 강도표 조건과 동일) + 듀얼 1패스**(재점검 포함) — 높음이어도 3루프를 적용하지 않고(문서에는 green이 없어 종료 조건이 오지 않는다 — 실측: 지적의 ~60%가 동기화류·~15%가 구현 접촉 전 판정 불가), blind 테스트 워커는 **비대상**이다(테스트 코드가 없는 산출물엔 그 계약이 성립하지 않는다 — 면제가 아니라 부적용). ⚠️ **분기 기준은 산출물 유형이지 경로가 아니며**(docs가 repo 루트인 프로젝트에서 경로 규칙이 우회된 실측), **그 자체가 실행 효력을 갖는 문서**(정책·runbook·설정 명세)와 **혼합 변경**(실행물+문서 한 작업)은 실행물 규칙을 따른다.
