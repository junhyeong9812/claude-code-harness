...
- **구현 사다리 (Ponytail 이식 — MIT, DietrichGebert/ponytail)**: 문제와 변경이 닿는 코드·실제 흐름을 **끝까지 읽은 뒤**, 처음 성립하는 단에서 멈춘다 — ①필요한가(추측성 필요=생략·1줄 보고) ②이 코드베이스에 이미 있나(재사용) ③stdlib ④플랫폼 기본 기능(DB 제약·CSS 등) ⑤설치된 의존성(새 의존성은 최후) ⑥한 줄 ⑦그제야 동작하는 최소 코드. 요청 없는 추상화·보일러플레이트·'나중용' 스캐폴딩 금지, 삭제>추가, 같은 크기면 엣지케이스에 맞는 쪽. **버그는 근본 원인에서** — 고칠 함수의 호출처를 전수 grep해 공유 지점에서 1회. 알려진 상한이 있는 의도적 단순화는 `ponytail:` 주석(상한·업그레이드 경로). **게으름 비적용**: 문제 이해·신뢰경계 입력 검증·데이터 손실 방지·보안·접근성·명시 요청·검증(§4 안전선·강도표 그대로).
...
- ★ **[구현 검증] 이연 (설계 문서 한정)**: 근거가 구현 접촉에만 있는 판정(수치·비용·형식 세부)은 `[구현 검증]` 태그로 **이연할 수 있다** — 각 문서 태그 + **그 프로젝트의 중앙 대장**(`implementation-verification.md` 류 — 문서와 다른 repo면 그 repo의 커밋)에 등재해야 유효하다(무음 이연 금지). 판별 가이드: *"이 답이 틀렸다는 걸 문서만 읽고 알 수 있나?"* — 알 수 있으면 문서에서 판정한다(이연 남용 방지).
...
| 리뷰 | 셀프체크 | **듀얼 1패스**(Opus 워커 ∥ codex → 종합 → 감사 → post-fix 재점검 1회) | **듀얼 리뷰 루프**(≤3) + 설계 선검증(신설 불변식이 있을 때만) + blind 테스트 워커 |
| 테스트 설계 | 구현자 | 분리 패스(spec 먼저) | 별도 워커 — 구현 diff 미열람 계약 |

- ★ **산출물 유형 분기 (2026-08-06 실측 — jun-bank)**: 산출물이 **설계 문서** — *그 내용이 이후 코드로 구현되어 컴파일·테스트가 검증하게 될 문서*(설계·명세·계약) — 이면 리뷰 상한 = **설계 선검증(신설 불변식 덩어리 의무) + 듀얼 1패스**(재점검 포함) — 높음이어도 3루프를 적용하지 않고(문서에는 green이 없어 종료 조건이 오지 않는다 — 실측: 지적의 ~60%가 동기화류·~15%가 구현 접촉 전 판정 불가), blind 테스트 워커는 **비대상**이다(테스트 코드가 없는 산출물엔 그 계약이 성립하지 않는다 — 면제가 아니라 부적용). ⚠️ **분기 기준은 산출물 유형이지 경로가 아니며**(docs가 repo 루트인 프로젝트에서 경로 규칙이 우회된 실측), **그 자체가 실행 효력을 갖는 문서**(정책·runbook·설정 명세)와 **혼합 변경**(실행물+문서 한 작업)은 실행물 규칙을 따른다.
...
- **사이클 마감 2단 (검증 완료 *후* — 사전 예측 대신 사후 전망)**: ①프로젝트 `docs/plans/NEXT.md`(롤링 단일 문서, `templates/next.md`) 갱신 — 다음 작업 후보·발생 가능 문제·방법론 비교·대처·우선순위(자명 작업은 1행) ②log의 **CS 이슈**를 study-note `cs/issue`에 아카이브(`playbooks/issue-archive.md` — 0건이면 log 1행).
...
> v4.2 (2026-09-24): §4 구현 사다리(Ponytail) · 설계 선검증 조건화 · §7 사이클 마감 2단(NEXT.md·issue-archive) — `docs/plans/2026-09-24/ponytail-next-issue-archive/`.
