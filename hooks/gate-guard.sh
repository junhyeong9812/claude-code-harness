...
if [ "$IS_BASH" = "1" ]; then
  ...
  # 상태파일 Bash 쓰기 차단 — lazymode 아래 쓰기 패턴 전부(문자열 패턴으로 .events 예외를 두려 했으나
  # regex 토큰 경계≠셸 word-splitting(tab·>·심링크)으로 상태파일 쓰기 우회가 신설됨을 리뷰 loop1이 실증
  # (P0). detect-layer 는 훅 서브프로세스라 이 Bash 가드 대상이 아니므로 .events 예외 자체가 불필요.
