...
#
# ⚠ 한계 (문서화된 backstop, 완전 보장 아님):
#   훅 stdin 에는 Bash 프로세스 stdin 이 아니라 hook JSON(tool_input.command 문자열)만 들어온다.
#   따라서 `codex exec - < prompt.txt`·`printf %s "$SECRET" | codex exec -`·`codex exec "$(cmd)"` 처럼
#   파일 리다이렉트·파이프·명령치환으로 넘어가는 실제 바이트는 볼 수 없다. 이 훅은 명령 문자열에
#   시크릿이 인라인된 경우만 잡는 **backstop** 이다. 외부 전송 전 시크릿 0 매칭의 primary 보장은
#   절차 스캔(playbooks/review.md §5 — packet 파일을 repo 밖에 두고 스캔 후 호출)이다.
...
if [ -z "$COMMAND" ]; then
  # tool_input.command 를 못 뽑았는데 raw 에 codex 흔적이 있으면 판정 불가 → 경고만(차단은 FP 과함).
  if printf '%s' "$HOOK_INPUT" | grep -qi 'codex'; then
    echo "[codex-scan] 경고: 명령 파싱 실패 — codex 시크릿 스캔을 수행하지 못했습니다(절차 스캔으로 대체하세요)." >&2
  fi
  exit 0
fi
...
scan '(password|passwd|secret|token|api[_-]?key|access[_-]?key)[[:space:]]*[:=][[:space:]]*['"'"'"]?[^[:space:]'"'"'"]{8,}' '크리덴셜 할당(password/token/secret 등)'
