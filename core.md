...
- **`pair` (2축 매트릭스 밖 독립 모드)**: 정의·설계를 **순수 대화**로 사용자 스스로 도달하게 하고(가이드형 객관식 선제시 금지 — task.md 6칸은 라이브 append), 로직 구현은 **항상 사용자가 직접 타이핑**한다. Claude는 테스트/보일러플레이트 파일만 작성(gate-guard의 `is_test_file()` 패턴 매칭 파일 + 기존 docs/plans·상태파일 면제는 Edit/Write 허용, 그 외 로직 파일은 **Edit/Write/MultiEdit**을 PreToolUse에서 항상 차단 — Bash 쓰기는 §0.6 FP 근거로 하드 차단 없이 소프트 리마인더만, `*-write`와 동일 경계)하고 핑퐁 스타일로 리뷰한다 — TDD 사이클(테스트 케이스 1개=사이클 경계)로 요구사항 기반 엣지케이스를 하나씩 다룬다. 절차는 `playbooks/pair-coding.md`.
